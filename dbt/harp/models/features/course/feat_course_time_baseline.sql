...
{{ config(
  ...
  unique_key=['held_year', 'jyo_cd', 'distance_m', 'surface', 'track_cd', 'surface_condition_cd'],
  tags=['feature']
) }}
...
yearly_course_time as (
  select
    ...
    sum(time_sec * time_sec) as sum_time_sec_sq,
...
yearly_course_time_cum as (
  select
    ...
    sum(yc.race_count) over (
      partition by yc.jyo_cd, yc.distance_m, yc.track_cd, yc.surface, yc.surface_condition_cd
