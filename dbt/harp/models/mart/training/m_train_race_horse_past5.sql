...
cur_base as (
  ...
  from {{ ref('int_race_entry_enriched') }} l
  left join metrics m
...
joined_base as (
  select
    *
  from cur_base ha
  left join {{ ref('feat_course_profile') }} fc
    using (jyo_cd, distance_m, surface, gate_number, track_cd, held_year)
  left join {{ ref('feat_course_style_profile') }} fcs
    using (jyo_cd, distance_m, surface, track_cd, held_year)
  left join {{ ref('feat_race_relative_z') }} frz
