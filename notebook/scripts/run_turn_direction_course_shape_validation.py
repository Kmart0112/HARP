...
RUNS_CSV_PATH = (
    ...
    / "report"
    / "results"
    / f"{TODAY}_turn_direction_course_shape_runs.csv"
)
RUN_LOG_DIR = PROJECT_ROOT / "outputs" / f"turn_direction_course_shape_logs_{TODAY}"

TURN_DIRECTION_FEATURE = "same_turn_direction_avg_pos4_agari_synergy"
TURN_DIRECTION_SURFACE_FEATURE = "same_turn_direction_surface_avg_pos4_agari_synergy"
BASE_OVERLAP_FEATURE = "same_surface_condition_avg_pos4_agari_synergy"
TARGET_FEATURES = {
    TURN_DIRECTION_FEATURE,
    TURN_DIRECTION_SURFACE_FEATURE,
    BASE_OVERLAP_FEATURE,
}

SCENARIOS: list[tuple[str, set[str]]] = [
    ("baseline_existing", {BASE_OVERLAP_FEATURE}),
    ("turn_direction_only", {BASE_OVERLAP_FEATURE, TURN_DIRECTION_FEATURE}),
    (
        "turn_direction_surface_only",
        {BASE_OVERLAP_FEATURE, TURN_DIRECTION_SURFACE_FEATURE},
    ),
    (
        "both_add",
        {BASE_OVERLAP_FEATURE, TURN_DIRECTION_FEATURE, TURN_DIRECTION_SURFACE_FEATURE},
    ),
    ("replace_surface_condition", {TURN_DIRECTION_SURFACE_FEATURE}),
]
...
def ensure_required_lines_present() -> None:
    ...
    missing = [feature for feature in sorted(TARGET_FEATURES) if re.search(rf"^\s*(#\s*)?-\s*{re.escape(feature)}\s*$", text, re.MULTILINE) is None]
    if missing:
...
def ensure_baseline_feature_enabled() -> None:
    text = FEATURES_PATH.read_text(encoding="utf-8")
    if not _is_feature_enabled(text, BASE_OVERLAP_FEATURE):
        raise ValueError(f"required baseline feature is OFF in features.yml: {BASE_OVERLAP_FEATURE}")
