...
RUNS_CSV_PATH = (
    ...
    / "report"
    / "results"
    / f"{TODAY}_pos4_agari_wavg5_robust_vs_z_runs.csv"
)
RUN_LOG_DIR = PROJECT_ROOT / "outputs" / f"pos4_agari_wavg5_robust_vs_z_logs_{TODAY}"

Z_FEATURE = "pos4_agari_synergy_wavg5_recent_z"
ROBUST_FEATURE = "pos4_agari_synergy_wavg5_recent_robust"
TARGET_FEATURES = [Z_FEATURE, ROBUST_FEATURE]

SCENARIOS: list[tuple[str, set[str]]] = [
    ("baseline_z_only", {Z_FEATURE}),
    ("robust_only", {ROBUST_FEATURE}),
    ("z_plus_robust", {Z_FEATURE, ROBUST_FEATURE}),
]
...
def main() -> None:
    ...
    robust_row = next(row for row in rows if row["scenario"] == "robust_only")
    both_row = next(row for row in rows if row["scenario"] == "z_plus_robust")
    summary = {
        ...
        "robust_only": robust_row,
        "z_plus_robust": both_row,
