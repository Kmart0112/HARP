...
RUNS_CSV_PATH = (
    ...
    / "report"
    / "results"
    / f"{TODAY}_pace_front_disadvantage_past3_runs.csv"
)
RUN_LOG_DIR = PROJECT_ROOT / "outputs" / f"pace_front_disadvantage_past3_logs_{TODAY}"

P1_FEATURE = "p1_pace_front_disadvantage"
P2_FEATURE = "p2_pace_front_disadvantage"
P3_FEATURE = "p3_pace_front_disadvantage"
AVG_FEATURE = "pace_front_disadvantage_avg3"
TARGET_FEATURES = [P1_FEATURE, P2_FEATURE, P3_FEATURE, AVG_FEATURE]

SCENARIOS: list[tuple[str, set[str]]] = [
    ("baseline", set()),
    ("set_only", {P1_FEATURE, P2_FEATURE, P3_FEATURE}),
    ("avg_only", {AVG_FEATURE}),
]

RESTORE_ENABLED: set[str] = set()
...
def _to_row(scenario: str, enabled: set[str], metrics: Metrics, baseline: Metrics) -> dict[str, str]:
    delta_auc = metrics.auc - baseline.auc
    delta_logloss = metrics.logloss - baseline.logloss
    delta_brier = metrics.brier - baseline.brier
    decision = "基準" if scenario == "baseline" else decide(delta_auc, delta_logloss)
    return {
        "scenario": scenario,
...
def main() -> None:
    ...
    try:
        for scenario, enabled in SCENARIOS:
            ...
            rows.append(_to_row(scenario, enabled, metrics, baseline_metrics))
            print(
        ...
        try:
            apply_feature_states(RESTORE_ENABLED)
        except Exception:
    ...
    set_row = next(row for row in rows if row["scenario"] == "set_only")
    avg_row = next(row for row in rows if row["scenario"] == "avg_only")
    summary = {
        "runs_csv": str(RUNS_CSV_PATH.relative_to(PROJECT_ROOT)),
        "run_log_dir": str(RUN_LOG_DIR.relative_to(PROJECT_ROOT)),
        "set_only": set_row,
        "avg_only": avg_row,
        "restored_features_state": "new past3 features=OFF (raw pace_front_disadvantage is not in features.yml)",
    }
